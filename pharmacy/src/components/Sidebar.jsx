...
const NAV_ITEMS = [
  ...
  { icon: Building, label: "بيانات الصيدلية", path: "/pharmacy-info" },
  { icon: CalendarDays, label: "جدول العمل", path: "/schedule" },
  { icon: ClipboardList, label: "أدويتي", path: "/medications" },
  { icon: Database, label: "قائمة الأدوية", path: "/medications-list" },
...
function Sidebar({ isOpen, onToggle, navbarHeight = "81px" }) {
  ...
  return (
    <aside
      ...
      // ✅ تعديل العرض والـ Transform لكي تنزلق القائمة في الموبايل وتختفي بدلاً من ترك مساحة فارغة
        ...
        {NAV_ITEMS.map(({ icon: Icon, label, path }) => {
          ...
          return (
            <Link
              ...
              to={path}
              onClick={() => {
                // إغلاق القائمة في الموبايل تلقائياً عند اختيار صفحة
