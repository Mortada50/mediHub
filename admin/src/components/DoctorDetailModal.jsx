...
export default function DoctorDetailModal({
  ...
  const handleAction = (action) => {
    ...
    onToggleStatus(doctor._id, doctor.role, action, doctor.fullName);
        ...
        <div className="flex flex-col lg:flex-row gap-3 p-4">
          ...
          <div className="flex flex-col gap-3 flex-1 min-w-0">
            ...
            <div className="bg-white rounded-xl p-4">
              ...
              <div className="flex gap-2 mt-4 flex-wrap">
                <StatCard
                  icon={<Clock size={13} />}
                  label="مدة الحجز"
                  value={`${doctor.appointmentDuration ?? 30} دقيقة`}
                />
                <StatCard
                  icon={<Star size={13} className="text-yellow-400" />}
                  label="التقييم"
                  // todo: add doctor.rating ??
                  value={"4.9"}
                />
                <StatCard
                  icon={<Phone size={13} />}
                  label="الاستشارات"
                  value={doctor.consultations ?? 88}
                  sub="استشارة"
                />
                <StatCard
                  icon={<Users size={13} />}
                  label="الحجوزات"
                  value={`${doctor.bookings ?? 866} حجز`}
                  sub={`${doctor.cancelRate ?? 12}% إلغاء`}
                />
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl p-4">
              ...
              <div className="flex flex-col gap-2.5">
                ...
                <InfoRow label="تلفون العيادة" value={doctor.phone} dir="ltr" />
                <div className="flex gap-3">
                  <div className="flex-1">
                    <InfoRow label="المدينة" value={doctor.address?.city} />
                  </div>
                  <div className="flex-1">
                    <InfoRow label="المديرية" value={doctor.address?.area} />
                  </div>
                </div>
                <InfoRow label="الحي / الشارع" value={doctor.address?.street} />
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3 lg:w-[320px] shrink-0">
            ...
            <div className="bg-white rounded-xl p-4">
              ...
              <div className="flex flex-col gap-2.5">
                <InfoRow label="التخصص الطبي" value={doctor.speciality} />
                <InfoRow
